...
let respond protocol m =
  ...
  | Json_Request ->
      Worker.post_message 
        (Js.string 
          (X_ocaml_json.json_string_of_response m))
  | Ocaml_Request -> 
      Worker.post_message 
        (X_protocol.resp_to_bytes m)
...
let reformat protocol ~id code =
  let code' = 
    ...
    with err -> code in
    if code <> code' then 
      respond protocol (Formatted_source (id, code'));
    code'
...
let run () =
  ...
  | protocol, message -> (
    ...
    | Format_config conf -> 
        Ocamlfmt.configure conf
    | Format (id, code) -> 
        ignore (reformat protocol ~id code : string)
    | Eval (id, line_number, code) ->
        let code = reformat protocol ~id code in
        let output ~loc out =
          respond protocol (Top_response_at (id, loc, out)) in
